...
Rowan's NOTES:
Retrieved on 6/30/14 by me from https://code.launchpad.net/~godfryd/python-spc/main
I just copied this single file and didn't install the package.
USAGE:
import spc
Create a spc object, 
s = spc.Spc( <data_list>, <chart_type>, <rules-default_basic> ), for example
s = spc.Spc([1, 2, 3, 3, 2, 1, 3, 8], spc.CHART_X_MR_X, spc.RULES_WECO)
Then call 
s.get_stats() #(center, LCL, UCL)
s.get_violating_points(rules=[])
s.get_chart(ax=None) #can supply ax
After calling get_chart(), you need to call plt.show() to see chart or 
uncomment the line that controls this, ~line 429
This assumes that you have imported matplotlib.pyplot as plt

See below for available rules and charts
"""
